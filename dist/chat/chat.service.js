...
const rawData = __importStar(require("../data/mlas.json"));
let ChatService = ChatService_1 = class ChatService {
    ...
    getAnswer(question, lang = 'English') {
        const q = question.trim().toLowerCase();
        console.log('📩 Question:', q);
        if (['portal available', '24 hours', 'anytime', '24/7', 'എപ്പോഴും ഉപയോഗിക്കാം', 'can i complaint in night', 'complaints at any time', 'complaint at any time', 'midnight complaint', 'after office hours', 'ഓഫീസ് സമയം കഴിഞ്ഞ്', 'രാത്രി ഫയൽ'].some(w => q.includes(w))) {
        ...
        if (['contact mla directly', 'can i contact mla', 'talk to mla', 'mla contact', 'directly contact mla', 'എംഎൽഎയെ ബന്ധപ്പെടുക', 'mlaയെ നേരിട്ട് ബന്ധപ്പെടാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['hi', 'hello', 'hey', 'hai', 'hay', 'നമസ്കാരം', 'ഹലോ', 'good morning', 'good evening']
            .some(w => q.toLowerCase().split(/\s+/).includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['thank', 'നന്ദി'].some(w => q.includes(w))) {
            return lang === 'Malayalam' ? 'സന്തോഷം! 😊 മറ്റ് ചോദ്യങ്ങൾ ഉണ്ടെങ്കിൽ ചോദിക്കാം.' : 'You\'re welcome! 😊 Feel free to ask anything else.';
        }
        ...
        if (['book appointment', 'mla appointment', 'meet mla', 'appointment with mla', 'schedule meeting', 'in-person meeting', 'visit mla office', 'mla meeting request', 'എംഎൽഎയെ കാണണം', 'എംഎൽഎ അപ്പോയിന്റ്മെന്റ്', 'കൂടിക്കാഴ്ച', 'എംഎൽഎ മീറ്റിംഗ്', 'നേരിട്ട് കാണണം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['drainage blocked', 'drainage overflow', 'water logging', 'sewage overflow', 'drain blocked', 'rain water issue', 'overflowing drainage', 'drainage complaint', 'ഡ്രെയിനേജ് തടസ്സം', 'ഓട തടസ്സം',
            'വെള്ളക്കെട്ട്', 'മഴവെള്ള പ്രശ്നം', 'ചാലൊഴുക്ക് തടസ്സം', 'ഡ്രെയിനേജ് നിറഞ്ഞു'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['character certificate', 'caste certificate', 'income certificate', 'certificate signed by mla', 'mla signature',
            'document attestation', 'recommendation letter', 'സർട്ടിഫിക്കറ്റ്', 'ക്യാരക്ടർ സർട്ടിഫിക്കറ്റ്', 'ജാതി സർട്ടിഫിക്കറ്റ്', 'വരുമാന സർട്ടിഫിക്കറ്റ്', 'MLA ഒപ്പ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['scholarship', 'educational aid', 'student financial help', 'mla scholarship', 'education support', 'study assistance', 'വിദ്യാഭ്യാസ സ്കോളർഷിപ്പ്', 'സ്കോളർഷിപ്പ്',
            'വിദ്യാഭ്യാസ സഹായം', 'വിദ്യാർത്ഥി ധനസഹായം', 'MLA scholarship'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['housing scheme', 'house scheme', 'housing eligibility', 'government housing', 'life mission', 'pmay', 'വീട് പദ്ധതി', 'ഭവന പദ്ധതി', 'ലൈഫ് മിഷൻ',
            'വീട് ലഭിക്കുമോ', 'ഹൗസിംഗ് സ്കീം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['government site', 'സർക്കാർ', 'government', 'run by', 'കേരള സർക്കാർ ഔദ്യോഗിക വെബ്സൈറ്റ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['government pension', 'pension scheme', 'apply pension', 'widow pension', 'പെൻഷൻ', 'വിധവ പെൻഷൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['medical aid', 'treatment help', 'financial aid for treatment', 'medical assistance', 'ചികിത്സ സഹായം', 'മെഡിക്കൽ സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['school fee', 'education support', 'student fee help', 'സ്കൂൾ ഫീസ്', 'വിദ്യാഭ്യാസ സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['ration card correction', 'ration card issue', 'ration complaint', 'റേഷൻ കാർഡ്', 'റേഷൻ പ്രശ്നം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['disability support', 'disabled help', 'wheelchair support', 'ഭിന്നശേഷി സഹായം', 'വ്യത്യസ്ത ശേഷി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['unemployment assistance', 'jobless support', 'job scheme', 'തൊഴിലില്ലായ്മ', 'ജോലി സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['recommendation letter', 'mla recommendation', 'reference letter', 'ശുപാർശ കത്ത്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['agriculture subsidy', 'farmer scheme', 'farming support', 'കാർഷിക സബ്സിഡി', 'കർഷക സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['emergency financial support', 'poor family help', 'financial crisis', 'സാമ്പത്തിക സഹായം', 'അടിയന്തിര സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['scheme application status', 'welfare status', 'application tracking', 'അപേക്ഷ നില', 'സ്കീം സ്റ്റാറ്റസ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['laptop scheme', 'study aid', 'student laptop', 'ലാപ്ടോപ്പ് പദ്ധതി', 'പഠന സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['house repair', 'home maintenance aid', 'repair assistance', 'വീട് അറ്റകുറ്റപ്പണി', 'വീട് സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['senior citizen support', 'elderly assistance', 'old age help', 'മുതിർന്ന പൗരൻ', 'വൃദ്ധ സഹായം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['disability pension', 'pension for disabled', 'ഭിന്നശേഷി പെൻഷൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['small business support', 'self employment', 'startup help', 'സ്വയംതൊഴിൽ', 'ചെറുകിട ബിസിനസ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['welfare payment delay', 'scheme payment pending', 'payment issue', 'പദ്ധതി തുക വൈകുന്നു', 'പേയ്മെന്റ് പ്രശ്നം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['water connection', 'drinking water connection', 'new water supply', 'കുടിവെള്ള കണക്ഷൻ', 'വെള്ള കണക്ഷൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['new government schemes', 'latest schemes', 'government benefits', 'പുതിയ സർക്കാർ പദ്ധതി', 'സർക്കാർ ആനുകൂല്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['education loan', 'student loan help', 'loan assistance', 'വിദ്യാഭ്യാസ വായ്പ', 'വിദ്യാർത്ഥി വായ്പ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['character certificate', 'caste certificate', 'income certificate', 'certificate request', 'ക്യാരക്ടർ സർട്ടിഫിക്കറ്റ്', 'ജാതി സർട്ടിഫിക്കറ്റ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['create account', 'create an account', 'register a account', 'register an account', 'signup', 'അക്കൗണ്ട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['otp not', 'otp is not', 'receive an otp', 'receive a otp', 'otp varunnilla', 'ഒടിപി', 'otp വരുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what if resolved but problem exists', 'complaint status says', 'resolved but', 'fake resolved', 'wrongly resolved', 'problem still there', 'പ്രശ്നം ഇപ്പോഴും', 'resolved ആയെങ്കിലും'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if ((['detail', 'details are mandatory', 'information', 'require', 'what do i need', 'document', 'mandatory', 'വിവരങ്ങൾ', 'എന്തൊക്കെ വേണം'].some(w => q.includes(w))) &&
            (['need to submit', 'grievance', 'issue', 'പരാതി'].some(w => q.includes(w)))) {
            return lang === 'Malayalam'
        ...
        if (['types of complaint', 'type of complaints', 'kind of complaint', 'what complaint', 'which complaint', 'വിഭാഗം'].some(w => q.includes(w))) {
            return lang === 'Malayalam' ? 'ഈ വിഷയങ്ങളിൽ പരാതി നൽകാം:\n• റോഡ്, ഗതാഗതം\n• ജലവിതരണം\n• വൈദ്യുതി\n• പൊതുജനാരോഗ്യം\n• ഭവനം\n• വിദ്യാഭ്യാസം\n• മറ്റ് പൗര പ്രശ്നങ്ങൾ'
        ...
        if (['check status', 'updates', 'complaint was submitted successfully', 'പരാതി വിജയകരമായി', 'track', 'status engane', 'complaint status', 'handling my complaint'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['department', 'vakuppu', 'who handles', 'ഏത് വകുപ്പ്'].some(w => q.includes(w)) && ['complaint', 'handle', 'route', 'parathi', 'പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['status', 'track', 'സ്റ്റാറ്റസ്', 'സ്ഥിതി'].some(w => q.includes(w)) && ['grievance', 'issue', 'പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['know if', 'epol pariharikum', 'എങ്ങനെ അറിയാം'].some(w => q.includes(w)) && ['complaint', 'status', 'parathi', 'പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['who is my mla', 'contact mla', 'mla number', 'ഫോൺ നമ്പർ'].some(w => q.includes(w))) {
            ...
            return lang === 'Malayalam'
                ? `കേരളത്തിൽ ആകെ 140 നിയമസഭാ മണ്ഡലങ്ങൾ ഉണ്ട്:\n\n` +
                    Object.entries(DISTRICTS).map(([d, v]) => `${d.charAt(0).toUpperCase() + d.slice(1)}: ${v.ml.join(', ')}`).join('\n')
                : `Kerala has 140 Assembly constituencies across 14 districts:\n\n` +
                    Object.entries(DISTRICTS).map(([d, v]) => `${d.charAt(0).toUpperCase() + d.slice(1)} (${v.en.length}): ${v.en.join(', ')}`).join('\n');
        }
        ...
        if (['trust', 'useful', 'സുരക്ഷിത', 'ഉപയോഗ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['unable to upload files', 'cannot upload file', 'upload issue', 'upload problem', 'file upload error', 'അപ്‌ലോഡ്', 'ഫയൽ അപ്‌ലോഡ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is mla', 'who is mla', 'mla meaning', 'mla എന്നാൽ', 'mla ആരാണ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['difference', 'mp and mla', 'mla vs mp', 'mla mp വ്യത്യാസം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is grievance', 'grievance meaning', 'grievance എന്നാൽ', 'grievance definition'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['char', 'character limit', 'word limit', 'description limit', 'characters are allowed', 'എത്ര വാക്ക്', 'description length'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['tips', 'good complaint', 'write a complaint', 'how to write', 'better complaint', 'effective', 'നല്ല പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['fake complaint', 'false complaint', 'misuse', 'abuse', 'വ്യാജ പരാതി', 'ദുരുപയോഗം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['difference between', 'portal vs', 'entemla vs', 'other portal', 'jagratha', 'sevasindhu', 'grievance portal'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['forgot tracking', 'lost tracking id', 'tracking id mararnju', 'tracking id കിട്ടുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['internet', 'offline', 'without internet', 'ഇന്റർനെറ്റ് ഇല്ലാതെ', 'offline parathi'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['different constituency', 'two area', 'two constituency', 'complain for another area', 'two mla'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['no action', 'nothing happening', 'no progress', 'inert', 'department not responding', 'no change', 'നടപടിയില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['reminder', 'follow up', 'ഓർമ്മിപ്പിക്കൽ', 'ഫോളോ അപ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['voter id', 'aadhaar', 'id proof', 'document required to register', 'ആധാർ', 'വോട്ടർ ഐഡി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint visible to public', 'others can see', 'public complaint', 'public view', 'മറ്റുള്ളവർ കാണുമോ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what if mla rejects', 'mla dismiss', 'mla nirakarikkum', 'mla ignore my complaint', 'mla does not accept'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint limit per day', 'daily limit', 'per day', 'ഒരു ദിവസം', 'daily complaint'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is tracking id', 'tracking id meaning', 'tracking id എന്തിനാണ്', 'tracking id ആവശ്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to contact mla', 'meet mla', 'mla office visit', 'mla ഓഫീസ് സന്ദർശനം', 'mla-യെ കാണാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['portal down', 'website down', 'server error', 'error page', 'site not working', 'പോർട്ടൽ പ്രവർത്തിക്കുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can i suggest', 'feature request', 'improve portal', 'suggestion', 'feedback for portal', 'നിർദ്ദേശം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is corestone', 'corestone innovations', 'corestone company', 'corestone എന്താണ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['new user', 'first time', 'beginner', 'how to start', 'getting started', 'ആദ്യമായി', 'പുതിയ യൂസർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['street light', 'streetlight', 'lamp post', 'തെരുവ് വിളക്ക്', 'വിളക്ക് കത്തുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['drainage', 'sewage', 'flood', 'waterlogging', 'drain block', 'മഴവെള്ളം', 'ഡ്രെയിൻ', 'വെള്ളക്കെട്ട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['hospital', 'health center', 'medical', 'doctor', 'ambulance', 'ആശുപത്രി', 'ആരോഗ്യ കേന്ദ്രം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['street dog', 'dog issue', 'stray dog', 'dog complaint', 'street dogs', 'തെരുവ് നായ', 'നായ പ്രശ്നം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['garbage dumping', 'waste issue', 'public garbage', 'trash complaint', 'dumping waste', 'garbage problem', 'മാലിന്യം', 'മാലിന്യ നിക്ഷേപം', 'ചവറ്റുകുട്ടി പ്രശ്നം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['school', 'college', 'teacher', 'education', 'scholarship', 'സ്കൂൾ', 'കോളേജ്', 'അദ്ധ്യാപകൻ', 'വിദ്യാഭ്യാസം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['housing', 'home', 'shelter', 'building permit', 'ഭവനം', 'വീട്', 'ഭവന പദ്ധതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['ration', 'ration card', 'pds', 'food supply', 'ration shop', 'റേഷൻ', 'റേഷൻ കാർഡ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['pension', 'welfare', 'scheme', 'benefit', 'allowance', 'പെൻഷൻ', 'ക്ഷേമ പദ്ധതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['tree', 'fallen tree', 'encroachment', 'land issue', 'public property', 'മരം', 'മരം വീണ്', 'കൈയേറ്റം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['noise', 'pollution', 'garbage', 'waste dump', 'smell', 'ശബ്ദ മലിനീകരണം', 'മാലിന്യം', 'ദുർഗന്ധം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['bus', 'transport', 'ksrtc', 'auto', 'public transport', 'ബസ്', 'ഗതാഗതം', 'ksrtc'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['bridge', 'culvert', 'overbridge', 'potholes', 'road damage', 'പാലം', 'കുഴി', 'റോഡ് തകർന്ന്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['view all mla', 'list of mla', 'all mla', 'mla list kerala', 'mla ലിസ്റ്റ്', 'എല്ലാ mla'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is dashboard', 'dashboard meaning', 'my dashboard', 'ഡാഷ്ബോർഡ്', 'dashboard എന്തിനാണ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['change language', 'switch language', 'english to malayalam', 'malayalam to english', 'ഭാഷ മാറ്റാൻ', 'language change'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['password strength', 'strong password', 'password rules', 'password requirement', 'പാസ്‌വേഡ് നിബന്ധന'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
                ? 'ശക്തമായ പാസ്‌വേഡ് ഉണ്ടാക്കാൻ:\n• കുറഞ്ഞത് 8 അക്ഷരങ്ങൾ\n• ഒരു Capital Letter\n• ഒരു Number\n• ഒരു Special Character (@, #, ! തുടങ്ങിയവ)'
                : 'For a strong password:\n• Minimum 8 characters\n• At least one capital letter\n• At least one number\n• At least one special character (@, #, !, etc.)';
        }
        if (['delete account', 'close account', 'remove account', 'അക്കൗണ്ട് ഡിലീറ്റ്', 'അക്കൗണ്ട് ക്ലോസ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint not submitted', 'submission failed', 'error while submitting', 'submit ആകുന്നില്ല', 'പരാതി പോകുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what happens to my data', 'data usage', 'data policy', 'personal information usage', 'ഡേറ്റ എന്ത് ചെയ്യും', 'വ്യക്തിഗത വിവരം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is janasamparkan', 'janasamparkan meaning', 'public outreach', 'ജനസമ്പർക്ക', 'jana samparkan'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['wrong category', 'selected wrong', 'category mistake', 'wrong type', 'തെറ്റായ കാറ്റഗറി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['number of complaints resolved', 'how many resolved', 'success rate', 'portal statistics', 'എത്ര പരാതി പരിഹരിച്ചു'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['session expired', 'logged out automatically', 'auto logout', 'സെഷൻ', 'ഓട്ടോ ലോഗ്ഔട്ട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['change password', 'update password', 'new password', 'പാസ്‌വേഡ് മാറ്റണം', 'പുതിയ പാസ്‌വേഡ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint on behalf of group', 'community complaint', 'group complaint', 'village complaint', 'സമൂഹ പരാതി', 'കൂട്ടായ പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint during election', 'election time', 'model code', 'election complaint', 'തിരഞ്ഞെടുപ്പ് സമയം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['print complaint', 'download complaint', 'complaint copy', 'print receipt', 'പരാതി പ്രിന്റ്', 'കോപ്പി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['share complaint', 'share tracking', 'send complaint link', 'complaint link', 'പരാതി ഷെയർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['government holiday', 'holiday', 'public holiday', 'complaint on holiday', 'അവധി ദിവസം', 'ഹോളിഡേ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is pwD', 'pwd full form', 'kwa full form', 'kseb full form', 'pwd meaning', 'kwa meaning'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint not visible', 'complaint disappeared', 'missing complaint', 'complaint kanunnilla', 'പരാതി കാണുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to logout', 'sign out', 'log out', 'ലോഗ് ഔട്ട്', 'സൈൻ ഔട്ട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['tablet', 'ipad', 'laptop', 'desktop', 'computer', 'ടാബ്ലറ്', 'ലാപ്ടോപ്', 'കമ്പ്യൂട്ടർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to find mla', 'search mla', 'find my mla', 'locate mla', 'mla കണ്ടെത്താൻ', 'ഏത് mla'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint priority', 'urgent complaint', 'high priority', 'priority complaint', 'അടിയന്തിര പരാതി', 'പ്രാധാന്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['account suspended', 'account blocked', 'account locked', 'account ban', 'അക്കൗണ്ട് ബ്ലോക്ക്', 'സസ്പെൻഡ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how is complaint verified', 'who verifies', 'verification process', 'check complaint', 'പരാതി ആര് പരിശോധിക്കും', 'വെരിഫിക്കേഷൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can mla reject', 'mla refuse', 'mla will not help', 'mla ignore', 'mla സഹായിക്കുന്നില്ല', 'mla നിരസിക്കും'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['best time to submit', 'when to submit', 'submit complaint timing', 'right time', 'ഏത് സമയം', 'ശരിയായ സമയം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is otp', 'otp meaning', 'otp full form', 'otp എന്താണ്', 'otp എന്നാൽ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint in english', 'website in english', 'submit in english', 'english complaint', 'ഇംഗ്ലീഷിൽ പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint in malayalam', 'website in malayalam', 'submit in malayalam', 'malayalam complaint', 'മലയാളത്തിൽ പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to attach photo', 'upload image', 'add photo', 'photo attach', 'ഫോട്ടോ എങ്ങനെ', 'ചിത്രം ചേർക്കാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['wrong phone number', 'registered wrong number', 'wrong mobile', 'തെറ്റായ നമ്പർ', 'wrong number registered'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['water supply problem', 'no water', 'water not coming', 'pipe leak', 'കുടിവെള്ളം', 'വെള്ളം വരുന്നില്ല', 'പൈപ്പ് ലീക്ക്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['power cut', 'no electricity', 'transformer issue', 'low voltage', 'current illa', 'വൈദ്യുതി മുടക്കം', 'കറന്റ് ഇല്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint response time', 'how fast', 'quick response', 'fast resolution', 'വേഗം പരിഹരിക്കും', 'എത്ര വേഗം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what if department wrong', 'forwarded to wrong dept', 'wrong department', 'wrong dept', 'തെറ്റായ വകുപ്പ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to write location', 'location details', 'address format', 'location format', 'സ്ഥലം എങ്ങനെ', 'വിലാസം എഴുതാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['government scheme complaint', 'pradhan mantri', 'central scheme', 'central government', 'കേന്ദ്ര സർക്കാർ', 'പ്രധാൻ മന്ത്രി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['who sees my complaint', 'complaint visibility', 'who can access', 'ആർക്ക് കാണാം', 'പരാതി ആർ കാണും'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint category list', 'all categories', 'available categories', 'list of categories', 'കാറ്റഗറി ലിസ്റ്റ്', 'എല്ലാ കാറ്റഗറി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
                ? 'EnteMLA-ൽ ലഭ്യമായ കാറ്റഗറികൾ:\n• PWD (റോഡ്, പാലം)\n• KWA (കുടിവെള്ളം)\n• KSEB (വൈദ്യുതി)\n• Health (ആരോഗ്യം)\n• Education (വിദ്യാഭ്യാസം)\n• Housing (ഭവനം)\n• Administrative / Others'
                : 'Available complaint categories in EnteMLA:\n• PWD (Roads, Bridges)\n• KWA (Water Supply)\n• KSEB (Electricity)\n• Health\n• Education\n• Housing\n• Administrative / Others';
        }
        if (['no smartphone', 'basic phone', 'keypad phone', 'feature phone', 'സ്മാർട്ട്ഫോൺ ഇല്ല', 'കീപാഡ് ഫോൺ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['akshaya', 'csc', 'common service centre', 'akshaya centre', 'അക്ഷയ', 'അക്ഷയ സെന്റർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['senior citizen', 'elderly', 'old age', 'disabled', 'specially abled', 'മുതിർന്ന പൗരൻ', 'വൃദ്ധർ', 'വൃദ്ധ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how portal helps mla', 'benefit for mla', 'mla benefit', 'mla portal use', 'mla എങ്ങനെ ഉപകരിക്കും'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['portal future', 'new features coming', 'upcoming features', 'roadmap', 'future update', 'പുതിയ ഫീച്ചർ', 'ഭാവി പദ്ധതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how many complaints per mla', 'mla complaint count', 'mla statistics', 'mla performance', 'mla എത്ര പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint acknowledgement', 'confirmation message', 'submission confirmation', 'complaint received message', 'പരാതി ലഭിച്ചു', 'confirm message'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['report chatbot issue', 'chatbot wrong answer', 'chatbot error', 'bot not working', 'ചാറ്റ്ബോട്ട് തെറ്റ്', 'ബോട്ട് പ്രവർത്തിക്കുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is ward', 'ward meaning', 'ward number', 'ward എന്താണ്', 'വാർഡ് നമ്പർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for road accident', 'accident prone area', 'accident spot', 'dangerous road', 'അപകടം', 'അപകട പ്രദേശം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['illegal construction', 'unauthorized building', 'building violation', 'encroach', 'അനധികൃത നിർമ്മാണം', 'കൈയ്യേറ്റ നിർമ്മാണം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['stray dog', 'stray animal', 'animal menace', 'dog bite', 'തെരുവ് നായ', 'മൃഗ ശല്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['public toilet', 'sanitation', 'open defecation', 'toilet facility', 'പൊതു ടോയ്‌ലറ്റ്', 'ശുചിത്വം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['internet connection problem', 'wifi issue', 'data not working', 'slow internet', 'ഇന്റർനെറ്റ് ഇല്ല', 'ഡേറ്റ പോകുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for corruption in school', 'teacher absent', 'school fund misuse', 'school corruption', 'സ്കൂൾ അഴിമതി', 'അദ്ധ്യാപകൻ വരുന്നില്ല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['manglish', 'type in manglish', 'manglish support', 'മംഗ്ലീഷ്', 'manglish parathi'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for missing person', 'person missing', 'child missing', 'missing report', 'കാണാതായ വ്യക്തി', 'കുട്ടി കാണാതായി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint against police', 'police misconduct', 'police complaint', 'പോലീസ് പരാതി', 'പോലീസ് ദുർനടപ്പ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for flood relief', 'disaster relief', 'natural disaster', 'flood', 'landslide', 'വെള്ളപ്പൊക്കം', 'ദുരന്ത ദുരിതാശ്വാസം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to give good description', 'describe problem', 'what to write in description', 'description tips', 'description എങ്ങനെ', 'വിവരണം എഴുതാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is pwD complaint', 'road complaint process', 'pwd process', 'pwd parathi', 'PWD പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['night complaint', 'late night', 'submit at night', 'രാത്രി പരാതി', 'രാത്രിയിൽ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for park', 'playground', 'public space', 'park maintenance', 'പൊതു ഇടം', 'കളിസ്ഥലം', 'പാർക്ക്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for market', 'shop', 'price hike', 'overpricing', 'market issue', 'കട', 'മാർക്കറ്റ്', 'വില കൂടുതൽ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for bridge repair', 'bridge damage', 'bridge broken', 'പാലം തകർന്നു', 'പാലം അപകടം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint confidential', 'is my name shown', 'name visible', 'complaint anonymous to department', 'പേര് കാണിക്കുമോ', 'രഹസ്യ പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can i add more info', 'add details later', 'update complaint', 'add information', 'കൂടുതൽ വിവരം', 'ഇൻഫർമേഷൻ ചേർക്കാൻ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is mla office', 'mla office meaning', 'mla office work', 'mla office ചെയ്യുന്നത്', 'mla ഓഫീസ് ജോലി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is constituency', 'constituency meaning', 'constituency എന്താണ്', 'മണ്ഡലം എന്നാൽ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for cemetery', 'burial ground', 'crematorium', 'funeral', 'ശ്മശാനം', 'മഖ്ബറ', 'ദഹന ശാല'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for fishing', 'fisherman problem', 'fish market', 'boat jetty', 'മത്സ്യബന്ധനം', 'മത്സ്യത്തൊഴിലാളി', 'ബോട്ട് ജെട്ടി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for library', 'public library', 'reading room', 'ഗ്രന്ഥശാല', 'വായനശാല', 'ലൈബ്രറി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for bus stop', 'bus shelter', 'waiting shed', 'ബസ് ഷെൽറ്റർ', 'കാത്തിരിപ്പ് കേന്ദ്രം', 'ബസ് സ്റ്റോപ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how does entemla work', 'portal working', 'how portal works', 'entemla process', 'portal process', 'entemla എങ്ങനെ പ്രവർത്തിക്കുന്നു'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can i file complaint for neighbor', 'complaint for others area', 'different house', 'neighbor problem', 'അയൽവാസി', 'അടുത്ത വീട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for sports', 'playground facility', 'sports ground', 'stadium', 'സ്പോർട്സ്', 'കായിക സൗകര്യം', 'സ്റ്റേഡിയം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for orphanage', 'old age home', 'destitute home', 'അനാഥാലയം', 'വൃദ്ധസദനം', 'ഭവനരഹിത കേന്ദ്രം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for temple', 'mosque', 'church', 'religious place', 'ക്ഷേത്രം', 'മസ്ജിദ്', 'പള്ളി', 'ആരാധനാലയം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can student use', 'student complaint', 'student file complaint', 'student portal', 'വിദ്യാർത്ഥി', 'സ്കൂൾ കുട്ടി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['website link', 'entemla url', 'entemla website', 'portal link', 'entemla address', 'വെബ്സൈറ്റ് ലിങ്ക്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['social media', 'facebook', 'instagram', 'twitter', 'youtube', 'സോഷ്യൽ മീഡിയ', 'ഫേസ്ബുക്ക്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for canal', 'irrigation', 'water canal', 'canal block', 'കനാൽ', 'ജലസേചനം', 'തോട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for footpath', 'footpath damaged', 'pavement', 'walking path', 'ഫുട്പാത്ത്', 'നടപ്പാത', 'ഫുൾ പ്ലാത്ത്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to register land complaint', 'land dispute', 'land problem', 'property dispute', 'ഭൂമി തർക്കം', 'ഭൂ പ്രശ്നം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for water tank', 'overhead tank', 'water storage', 'tank overflow', 'വാട്ടർ ടാങ്ക്', 'ജലസംഭരണി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for noise pollution', 'loudspeaker', 'loud music', 'construction noise', 'ശബ്ദ മലിനീകരണം', 'ലൗഡ്സ്പീക്കർ', 'ഒച്ച ശല്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how is entemla different', 'why use entemla', 'advantage of entemla', 'entemla benefit', 'entemla ഉപയോഗിക്കണം', 'entemla നല്ലത്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for well', 'drinking well', 'public well', 'well contamination', 'കിണർ', 'കുടിവെള്ള കിണർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for cemetery', 'burial', 'graveyard', 'ശ്മശാനം', 'കബർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what if i move', 'shifted house', 'changed address', 'new address', 'വിലാസം മാറി', 'പുതിയ വീട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how to report open wire', 'hanging wire', 'electric wire', 'dangerous wire', 'കമ്പി ഞാത്തിക്കിടക്കുന്നു', 'ഇലക്ട്രിക് വയർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for toxic waste', 'chemical waste', 'factory pollution', 'industrial waste', 'വ്യവസായ മാലിന്യം', 'ഫാക്ടറി മലിനീകരണം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for footover bridge', 'over bridge', 'pedestrian bridge', 'foot over bridge', 'കാൽനട പാലം', 'ഫുട്ഓവർ ബ്രിഡ്ജ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['tribal', 'adivasi', 'sc st complaint', 'scheduled caste', 'scheduled tribe', 'ആദിവാസി', 'പട്ടികജാതി', 'പട്ടികവർഗ്ഗം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for pollution in river', 'river pollution', 'lake pollution', 'water pollution', 'നദി മലിനീകരണം', 'കായൽ', 'ജല മലിനീകരണം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['can i see other complaints', 'view all complaints', 'public complaints', 'others complaint', 'മറ്റുള്ളവരുടെ പരാതി', 'പൊതു പരാതി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is mla fund', 'mla lad fund', 'mla development fund', 'lad fund', 'MLA ഫണ്ട്', 'വികസന ഫണ്ട്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what happens if complaint is spam', 'spam complaint', 'irrelevant complaint', 'complaint misuse', 'spam parathi', 'വ്യാജ പരാതി ഫലം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for footpath encroachment', 'shop encroachment', 'vendor blocking road', 'hawker', 'കട കൈയ്യേറ്റം', 'തെരുവ് കച്ചവടക്കാർ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for wifi', 'broadband issue', 'internet in panchayat', 'public wifi', 'പൊതു wifi', 'ഇന്റർനെറ്റ് ദൗർലഭ്യം'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for pipeline', 'gas pipeline', 'lpg supply', 'gas leakage', 'പൈപ്പ്ലൈൻ', 'ഗ്യാസ് ലീക്ക്', 'lpg'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for disability', 'wheelchair access', 'ramp', 'disabled facility', 'ഭിന്നശേഷി', 'വ്യീൽചെയർ', 'റാമ്പ്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['is entemla secure', 'private', 'portal security', 'data breach', 'hacker', 'portal safe', 'EnteMLA സുരക്ഷ', 'ഡേറ്റ സുരക്ഷ'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['complaint for ambulance', 'emergency vehicle', 'ambulance road', '108', 'ആംബുലൻസ്', 'ആപത്ത്'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['how many times can i track', 'track limit', 'tracking limit', 'tracking attempts', 'ട്രാക്ക് ലിമിറ്റ്', 'ട്രാക്കിംഗ് പരിധി'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
        ...
        if (['what is the vision of entemla', 'entemla goal', 'entemla mission', 'purpose of entemla', 'entemla ലക്ഷ്യം', 'entemla vision'].some(w => q.includes(w))) {
            return lang === 'Malayalam'
